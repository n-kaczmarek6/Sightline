...
function sectionParagraphs(title, text) {
  ...
  return [
    new Paragraph({ text: title, heading: HeadingLevel.HEADING_2, spacing: { before: 160, after: 60 } }),
    ...lines.map(
      (line) =>
        new Paragraph({
...
function chipSectionParagraphs(title, text) {
  const chip = chipParagraph(text);
  if (!chip) return [];
  return [new Paragraph({ text: title, heading: HeadingLevel.HEADING_2, spacing: { before: 160, after: 60 } }), chip];
}
