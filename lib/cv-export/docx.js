      ...
      new TableRow({
        children: [
          new TableCell({
            ...
            borders: NO_BORDERS,
            children: [new Paragraph({ text: left ? `– ${left}` : "", spacing: { after: 30 } })],
          }),
          new TableCell({
            width: { size: 50, type: WidthType.PERCENTAGE },
            borders: NO_BORDERS,
            children: [new Paragraph({ text: right ? `– ${right}` : "", spacing: { after: 30 } })],
          }),
        ],
      })
    );
...
function inlineListSectionParagraphs(title, text) {
  const table = inlineListTable(text);
  if (!table) return [];
  return [sectionHeading(title), table, new Paragraph({ text: "", spacing: { after: 40 } })];
}
